...
public class TestRecipesControllerDelete
{
  ...
  public void TestDelete(string entryName, List<Recipe> expected)
  {
    ...
    controllerInstance.Delete(entryName);

    var recipesWithoutrecipeDelete = controllerInstance.Get() as OkObjectResult;
    recipesWithoutrecipeDelete.Value.Should().BeEquivalentTo(expected);
