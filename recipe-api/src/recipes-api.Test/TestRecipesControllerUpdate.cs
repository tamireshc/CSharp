...
public class TestRecipesControllerUpdate
{
  public static IEnumerable<object[]> RecipeServiceTestData()
  {
    ...
    {       "Coxinha",
            new Recipe {
                        ...
                        PreparationTime = 0.4,
                        Ingredients = new List<string> {"10 xícaras de trigo", "4 xícaras de leite", "1 caldo de galinha", "1/2 colher de margarina"},
  ...
  {
    var serviceInstance = new RecipeService();
    var controllerInstance = new RecipesController(serviceInstance);

    controllerInstance.Update(entryName, entryRecipe);
    var allRecipesUpdated = controllerInstance.Get() as OkObjectResult;
    allRecipesUpdated.Value.Should().BeEquivalentTo(expected);
  }
}